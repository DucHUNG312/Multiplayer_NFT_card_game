...
const Battle = () => {
  ...
  useEffect(() => {
    const checkForPlayerInBattle = async () => {
      const playersInBattle = await contract.getBattle(battleName);

      if (
        !(walletAddress === playersInBattle.players[0]) &&
        !(walletAddress === playersInBattle.players[1])
      )
        navigate("/create-battle");
    };

    if (contract) checkForPlayerInBattle();
  }, [battleName, gameData]);
